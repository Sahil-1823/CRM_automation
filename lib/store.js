...
export async function saveEvent(event) {
  const record = {
    ...
    channel: event.channel || "heyreach",
    ...event,
  };
...
function matchesHeyreachWorkspace(event, heyreachAccountId) {
  if (!heyreachAccountId) return true;
  const tag = event.heyreachAccountId || "default";
  if (tag === heyreachAccountId) return true;
  return !event.heyreachAccountId && heyreachAccountId === "default";
}

export async function listEvents({ limit = 100, status, channel, heyreachAccountId } = {}) {
  let records = [];
  ...
  if (redis) {
    ...
    if (heyreachAccountId) {
      for (const id of order) {
        ...
        const event = await redis.get(`${EVENT_PREFIX}${id}`);
        if (!event) continue;
        if (channel && (event.channel || "heyreach") !== channel) continue;
        if (status && event.status !== status) continue;
        if (!matchesHeyreachWorkspace(event, heyreachAccountId)) continue;
        records.push(event);
      }
      return records;
    }

    const ids = order.slice(0, limit * 3);
    if (ids.length) {
  ...
  if (channel) {
    records = records.filter((r) => (r.channel || "heyreach") === channel);
  }
  if (status) {
    records = records.filter((r) => r.status === status);
  }
  if (heyreachAccountId) {
    records = records.filter((r) => matchesHeyreachWorkspace(r, heyreachAccountId));
  }
...
export function serializeEvent(event) {
  ...
  const scheduling = draft.scheduling || null;
  ...
  const out = {
    ...
    autoResolvedAt: event.autoResolvedAt ?? null,
    autoResolvedReason: event.autoResolvedReason ?? null,
    messageType: event.messageType ?? null,
    draftProjectId: event.draftProjectId ?? null,
    heyreachAccountId: event.heyreachAccountId ?? null,
    heyreachAccountLabel: event.heyreachAccountLabel ?? null,
    sentiment: {
    ...
    gmail: event.gmail
      ? {
          accountEmail: event.gmail.accountEmail ?? null,
          threadId: event.gmail.threadId ?? null,
          messageId: event.gmail.messageId ?? null,
          fromEmail: event.gmail.fromEmail ?? null,
          fromName: event.gmail.fromName ?? null,
          subject: event.gmail.subject ?? null,
          snippet: event.gmail.snippet ?? null,
        }
      : null,
    draft: {
      reply: draft.reply ?? "",
      ...
      scheduling: scheduling
        ? {
            ...
            mode: scheduling.mode ?? null,
            requestedTime: scheduling.requestedTime ?? null,
            status: scheduling.status ?? null,
            calendlyLink: scheduling.calendlyLink ?? null,
            proposedSlots: (scheduling.proposedSlots ?? []).map((s) => ({
              start: s.start,
              end: s.end,
              label: s.label ?? null,
            })),
            pendingBook: scheduling.pendingBook
              ? {
                  start: scheduling.pendingBook.start,
                  end: scheduling.pendingBook.end,
                  title: scheduling.pendingBook.title,
                  label: scheduling.pendingBook.label ?? null,
                }
              : null,
            calendarEventId: scheduling.calendarEventId ?? null,
            meetLink: scheduling.meetLink ?? null,
            calendarError: scheduling.calendarError ?? null,
          }
        : null,
      agentTrace: (draft.agentTrace ?? []).map((t) => ({
        tool: t.tool,
        result: t.result
          ? {
              connected: t.result.connected,
              available: t.result.available,
              slots: t.result.slots?.map((s) => ({ start: s.start, end: s.end, label: s.label })),
...
const GMAIL_THREAD_PREFIX = "crm:gmail:thread:";

export async function findEventByGmailThread(accountEmail, threadId, { status } = {}) {
  if (!accountEmail || !threadId) return null;
  const redis = getRedis();
  if (redis) {
    const eventId = await redis.get(`${GMAIL_THREAD_PREFIX}${accountEmail.toLowerCase()}:${threadId}`);
    if (eventId) {
      const event = await getEvent(eventId);
      if (event && (!status || event.status === status)) return event;
    }
  }

  const events = await listEvents({ limit: MAX_EVENTS, channel: "gmail" });
  return (
    events.find(
      (event) =>
        event.gmail?.threadId === threadId &&
        event.gmail?.accountEmail?.toLowerCase() === accountEmail.toLowerCase() &&
