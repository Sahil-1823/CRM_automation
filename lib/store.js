...
export async function listEvents({ limit = 100, status, channel } = {}) {
  let records = [];
...
/** Move an event to the front of the global newest-first index. */
async function bumpEventToFront(id) {
  const redis = getRedis();
  if (redis) {
    const order = (await redis.get(INDEX_KEY)) || [];
    const next = [id, ...order.filter((x) => x !== id)].slice(0, MAX_EVENTS);
    await redis.set(INDEX_KEY, next);
    return;
  }

  const state = await readFileStore();
  state.order = [id, ...(state.order || []).filter((x) => x !== id)].slice(0, MAX_EVENTS);
  await writeFileStore(state);
}
