...
function schedulingGuidance(scheduling) {
  ...
  if (scheduling.mode === "calendly") {
    return (
      "Scheduling: lead wants to meet. Reply warmly and suggest they pick a time — do NOT propose specific dates/times. " +
      "A Calendly link will be appended automatically; do not paste any URL yourself. "
    );
  }
  return "Scheduling: help find a time without inventing specific slots unless provided. ";
}
...
export async function generateDraftReply({
  ...
  const userContent = [
    ...
    agentContext?.scheduling
      ? `Scheduling context:\n${JSON.stringify(agentContext.scheduling)}`
      : null,
    skipRag
  ...
  const basePrompt =
    ...
    schedulingGuidance(agentContext?.scheduling) +
