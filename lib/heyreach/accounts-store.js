...
export function getSyntheticDefaultAccount() {
  ...
  return {
    ...
    label: "Default (env)",
    apiKey,
    webhookSecret: process.env.HEYREACH_WEBHOOK_SECRET?.trim() || "",
    projectId: null,
    calendlyLink: null,
    isSynthetic: true,
    createdAt: null,
...
export async function listHeyReachAccounts({ includeSynthetic = true } = {}) {
  ...
  if (includeSynthetic) {
    ...
    if (synthetic && !accounts.some((a) => a.id === DEFAULT_ACCOUNT_ID)) {
      accounts.unshift(synthetic);
    }
  }
  return accounts;
}
...
export async function getHeyReachAccountBySecret(secret) {
  if (!secret) return null;
  const accounts = await listHeyReachAccounts({ includeSynthetic: false });
  for (const account of accounts) {
    if (account.webhookSecret && account.webhookSecret === secret) return account;
  }
  const synthetic = getSyntheticDefaultAccount();
  if (synthetic?.webhookSecret && synthetic.webhookSecret === secret) return synthetic;
  return null;
}
