...
/**
 * Manual override: sync latest conversation, then draft only when lead spoke last.
 * Status follows thread: pending_review only when last message is from the lead.
 * When last message is from us, draft reply is cleared (nothing to send).
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    // Last message from us (or dismissed/auto_resolved): no draft text.
    if (status !== "pending_review") {
      const updated = await updateEvent(id, {
        status,
        draft: clearDraftReply(event.draft),
        lead: {
          ...lead,
          conversation,
