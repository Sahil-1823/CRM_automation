...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      ...
      const heyreachAccountId = url.searchParams.get("heyreachAccountId") || undefined;
      const limit = Number(url.searchParams.get("limit") || 100);
      const events = await listEvents({ status, channel, limit, heyreachAccountId });
    ...
    if (req.method === "DELETE") {
      ...
      if (all !== "1") {
        return jsonResponse(res, 400, {
          error: "Missing confirmation. Use DELETE /api/dashboard/events?all=1",
        });
      }
