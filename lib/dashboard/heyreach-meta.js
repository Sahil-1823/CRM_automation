...
import { getHeyReachAccount, DEFAULT_ACCOUNT_ID } from "../heyreach/accounts-store.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const heyreachAccountId = url.searchParams.get("heyreachAccountId") || DEFAULT_ACCOUNT_ID;
    const account = await getHeyReachAccount(heyreachAccountId);
    const apiKey = account?.apiKey || null;
    const accountKey = account?.id || heyreachAccountId;

    const events = await listEvents({
      limit: 500,
      channel: "heyreach",
      heyreachAccountId: heyreachAccountId || undefined,
    });
