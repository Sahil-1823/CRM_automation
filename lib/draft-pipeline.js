...
} from "./scheduling/booking-link.js";
...
export function finalizeAgentDraftResult(result, { channel = null, conversation = null } = {}) {
  let reply = result.draft.reply;
  const scheduling = result.scheduling;
  const thread = conversation || [];

  if (scheduling?.intent && scheduling.appendLink !== false) {
    if (channel === "gmail" && scheduling.mode === "calendly") {
      const alreadyInThread =
        scheduling.alreadyOffered ||
        conversationHasBookingOffer(thread, scheduling.calendlyLink);
      if (!alreadyInThread) {
        reply = appendCalendlyLinkToReply(reply, scheduling.calendlyLink);
      }
    } else if (
      channel === "heyreach" &&
      scheduling.mode === "booking_link" &&
      scheduling.bookingLink
    ) {
      const alreadyInThread =
        scheduling.alreadyOffered ||
        conversationHasBookingOffer(thread, scheduling.bookingLink);
      if (!alreadyInThread) {
