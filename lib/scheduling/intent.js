...
import { getSchedulingDefaults } from "./config.js";
import { formatSlotLabel } from "./slots.js";
...
export async function parseSchedulingIntent({
  ...
  const { timezone, defaultDurationMin } = getSchedulingDefaults();
...
export function buildPendingBook({
  ...
  return {
    ...
    title,
    description,
    label: formatSlotLabel(start, end, getSchedulingDefaults().timezone),
  };
}
