...
function parseConversationThread(payload, { yourMessage, replyMessage }) {
  ...
  if (raw) {
    for (let i = 0; i < raw.length; i++) {
      ...
      thread.push({ from, text, at: messageTimestamp(msg) || undefined });
    }
  }

  if (!thread.length) {
    const fallbackAt = messageTimestamp(payload) || undefined;
    if (yourMessage) thread.push({ from: "us", text: yourMessage, at: fallbackAt });
    if (replyMessage) thread.push({ from: "lead", text: replyMessage, at: fallbackAt });
  }
