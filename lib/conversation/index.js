...
/** Newest message timestamp in a thread (either side). Ignores messages without `at`. */
export function latestThreadMessageAt(thread) {
  let best = "";
  let bestMs = -1;
  for (const msg of thread || []) {
    const at = typeof msg?.at === "string" ? msg.at.trim() : "";
    if (!at) continue;
...
/** Activity time for sidebar sort: last message clock, not last sync. */
export function eventLastMessageAt(event) {
  const thread = conversationFromEvent(event);
  return (
    latestThreadMessageAt(thread) ||
    event?.sentAt ||
    event?.sendResult?.sentAt ||
    event?.createdAt ||
    ""
  );
}

/** Empty draft text while keeping RAG/scheduling metadata. */
export function clearDraftReply(draft) {
...
export function buildConversationOnlyPatch(event, mergedConversation) {
  ...
  // No draft when we already spoke last (or dismissed / auto-resolved).
  if (status !== "pending_review") {
    patch.draft = clearDraftReply(event.draft);
  }
  return patch;
}
...
export function buildConversationSyncPatch(priorEvent, mergedConversation, parsedLead = {}) {
  ...
  // Draft only when awaiting our reply (lead spoke last).
  if (status !== "pending_review") {
    patch.draft = clearDraftReply(priorEvent.draft);
  }
