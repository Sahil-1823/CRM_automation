...
const PROJECT_SELECTION_TTL = 60 * 60 * 24; // 24 hours in seconds

/**
 * Use LLM to select the most relevant project for an incoming conversation.
 * Result is cached in Redis by conversationId (if provided) for 24 hours so
 * repeated draft regenerations don't trigger an extra LLM call each time.
 * Returns the matching project object, or null if no good match / no projects.
 *
 * @param {string} conversationContext - text summary of the conversation
 * @param {Array}  projects
 * @param {string|null} conversationId - optional Redis cache key
 */
export async function selectProject(conversationContext, projects, conversationId = null) {
  if (!projects?.length) return null;
  if (projects.length === 1) return projects[0];

  const redis = getRedis();
  const cacheKey = conversationId ? `crm:project-select:${conversationId}` : null;

  // Check cache
  if (cacheKey && redis) {
    const cached = await redis.get(cacheKey);
    if (cached?.projectId !== undefined) {
      return projects.find((p) => p.id === cached.projectId) || null;
    }
  }
  ...
  try {
    ...
    const matched = projects.find((p) => p.id === parsed.projectId) || null;

    // Store in cache
    if (cacheKey && redis) {
      await redis.set(cacheKey, { projectId: matched?.id || null }, { ex: PROJECT_SELECTION_TTL });
    }
