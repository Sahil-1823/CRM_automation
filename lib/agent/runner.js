...
async function resolveScheduling({
  ...
  if (isCalendlySchedulingMode()) {
    const scheduling = buildCalendlyScheduling(calendlyLinkOverride);
    agentTrace.push({ tool: "calendly", result: { mode: "calendly", link: scheduling.calendlyLink } });
    return scheduling;
  }

  // Google Calendar scheduling disabled — restore SCHEDULING_MODE=gcal to re-enable.
  return buildCalendlyScheduling(calendlyLinkOverride);
}
